...
class ELAY_License_Handler {
	...
	public function get_license_status_short( $output ) {
		...
		$licenses_tab_url = admin_url('edit.php?post_type=' . ELAY_KB_Handler::get_post_type( ELAY_KB_Core::DEFAULT_KB_ID ) . '&page=ep'.'kb-add-ons#licenses');
		$output_status = array();
		...
		if ( ! empty($stored_license_data['new_version']) ) {
			$output_status[] = sprintf( __( 'Please update %s to its latest version %s' , 'echo-knowledge-base' ), '<strong>' . self::PLUGIN_NAME . '</strong>', $stored_license_data['new_version'] ) .
								'<a href="' . $licenses_tab_url . '"> ' . __( 'Click here for details.', 'echo-knowledge-base' ) . '</a>';
		}
		...
		$output[self::PLUGIN_NAME] = $list . '</ul>' . '<br/>' . sprintf( __( 'Check current license status %s here. %s', 'echo-knowledge-base' ), '<a href="' . $licenses_tab_url . '" target="_blank">', '</a>' );
	...
	private function save_license( $license_key ) {
		...
		$result = ELAY_Utilities::save_wp_option( self::LICENSE_KEY_OPTION_ID, $license_key, true );
	...
	public function get_license_state( $force_refresh=false ) {
		...
		ELAY_Utilities::save_wp_option('elay_last_license_check', $stored_license_data, true);

		// store last known status
		ELAY_Utilities::save_wp_option( ELAY_License_Handler::ELAY_LICENSE_STATE, array( $status_msg, $license_state, $latest_add_on_version, $expiry_date ), true );
