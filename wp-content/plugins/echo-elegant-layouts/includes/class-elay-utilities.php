...
class ELAY_Utilities {
	...
	const ELAY_ADMIN_CAPABILITY = 'manage_options';
	...
	public static function get_bottom_notice_message_box($message, $title='', $type='success', $extra_html='' ) {
			...
			foreach ( $EZSQL_ERROR as $error ){
				...
				foreach ( $amgr_tables as $table_name ) {
					if ( !empty($error['error_str']) && strpos($error['error_str'], $table_name) !== false ) {
						...
						ELAY_Logging::add_log( 'Database error', $EZSQL_ERROR );
		...
		return
			"<div class='eckb-bottom-notice-message'>
				...
				" . $extra_html . "
				<div class='elay-close-notice epkbfa epkbfa-window-close'></div>
			</div>";
	}
		...
		<div id="<?php echo $values[ 'id' ]; ?>" class="epkb-dialog-box-form">
			...
			<div class="epkb-dbf__footer">
				...
				<div class="epkb-dbf__footer__accept <?php echo isset($values['accept_type']) ? 'epkb-dbf__footer__accept--'.$values['accept_type'] : 'epkb-dbf__footer__accept--success'; ?>">
					<span class="elay-accept-button elay-dbf__footer__accept__btn">
	...
	public static function retrieve_and_sanitize_form( $form, $all_fields_specs ) {
		...
		foreach( $submitted_fields as $submitted_key => $submitted_value ) {
			...
			if ( $field_type == ELAY_Input_Filter::WP_EDITOR ) {
				$sanitized_fields[$submitted_key] = wp_kses( $submitted_value, ELAY_Utilities::get_allowed_html_tags( ['all'] ) );
	...
	private static function post_sanitize( $key, $default = '', $value_type = 'text', $max_length = 0 ) {
		...
		if ( $value_type == 'text-area' ) {
			...
			$value = wp_kses( $_POST[$key], ELAY_Utilities::get_allowed_html_tags( ['all'] ) );
		} else {
	...
	private static function get_sanitize( $key, $default = '', $value_type = 'text', $max_length = 0 ) {
		...
		if ( $value_type == 'text-area' ) {
			...
			$value = wp_kses( $_GET[$key], ELAY_Utilities::get_allowed_html_tags( ['all'] ) );
		} else {
	...
	public static function ajax_verify_nonce_and_admin_permission_or_error_die( $wpnonce_name='_wpnonce_epkb_ajax_action' ) {
		...
		if ( ! current_user_can( self::ELAY_ADMIN_CAPABILITY ) ) {
	...
	public static function get_allowed_html_tags( $contexts=[] ) {
		...
		add_filter( 'safe_style_css', array( 'ELAY_Utilities', 'safe_inline_css_properties' ) );
