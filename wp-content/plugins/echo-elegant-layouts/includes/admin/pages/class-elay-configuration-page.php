...
class ELAY_Configuration_Page {
	...
	public function __construct() {
		add_action( ELAY_KB_Core::ELAY_CONFIG_SIDEBAR_INTRO_SETTINGS, array( 'ELAY_Configuration_Page', 'add_config_page_other_settings' ), 10, 2 );
	...
	public static function add_config_page_other_settings( $unsused, $kb_config ) {

		if ( $kb_config['kb_main_page_layout'] != ELAY_KB_Config_Layout_Sidebar::LAYOUT_NAME ) {
			return $unsused;
		}
	...
	private static function show_sidebar_intro_editor( $kb_config ) {
		...
		<form id="elay-config-page__intro-settings">
			...
			ELAY_KB_Core::submit_button( array( __( 'Save', 'echo-elegant-layouts' ), 'save_sidebar_intro_text', '', '', true, false, 'epkb-aibb-btn epkb-aibb-btn--blue' ) ); ?>
		</form><?php
	...
	public static function save_sidebar_intro_text() {
		...
		if ( empty(ELAY_Utilities::post('_wpnonce_elay_ajax_action')) || ! wp_verify_nonce( ELAY_Utilities::post('_wpnonce_elay_ajax_action'), '_wpnonce_elay_ajax_action' ) ) {
			ELAY_Utilities::ajax_show_error_die( __( 'You do not have permission to edit this knowledge base', 'echo-knowledge-base' )  . '(39)');
		}

		// ensure user has correct permissions
		if ( ! current_user_can( ELAY_Utilities::ELAY_ADMIN_CAPABILITY ) ) {
			ELAY_Utilities::ajax_show_error_die(__( 'You do not have permission to change access', 'echo-knowledge-base' ) . ' (29)');
		}
