...
final class Echo_Elegant_Layouts {
	...
	public static $needs_min_core_version = '4.6.0';
	...
	public static function instance( $dormant=false ) {
		...
		if ( isset( self::$instance ) || self::$instance instanceof Echo_Elegant_Layouts  ) {
	...
	private function setup_system() {
		...
		new ELAY_KB_Config_Layouts( true );
		new ELAY_Upgrades();
	...
	private function handle_action_request( $action ) {
		if ( in_array($action, array('epkb_load_editor', 'eckb_apply_editor_changes', 'eckb_editor_get_themes_list', 'eckb_theme_wizard_get_themes_v2', ELAY_KB_Core::ELAY_APPLY_SETUP_WIZARD_CHANGES)) ) {
			ELAY_KB_Editor_Config::register_editor_hooks();
			return;
		}
	}
	...
	private function handle_ajax_requests( $action ) {
		...
		// handle user switching articles in Sidebar Layout
		if ( $action == 'elay_get_article' ) {
			new ELAY_Layout_Sidebar();
			new ELAY_Layout_Sidebar_v2();
            return;
		}
		...
		if ( in_array($action,
				array(ELAY_KB_Core::ELAY_CHANGE_ARTICLE_PAGE_CONFIG_AJAX, ELAY_KB_Core::ELAY_CHANGE_MAIN_PAGE_CONFIG_AJAX,
			          ELAY_KB_Core::ELAY_CHANGE_ONE_PARAM_AJAX, ELAY_KB_Core::ELAY_CHANGE_ARTICLE_CATEGORY_SEQUENCE) ) ) {
			new ELAY_KB_Config_Layouts();
			new ELAY_Layout_Sidebar();
			new ELAY_Layout_Sidebar_v2();
			new ELAY_Layout_Grid();
		...
		if ( in_array($action, array(ELAY_KB_Core::ELAY_SAVE_KB_CONFIG_CHANGES, ELAY_KB_Core::ELAY_UPDATE_KB_EDITOR) )  ) {
			new ELAY_KB_Config_Controller();
			return;
		}
		...
		if ( $action == 'elay-search-kb' ) {  // user searching KB
			new ELAY_KB_Search();
			return;
		}
		...
		if ( in_array($action, array(ELAY_KB_Core::ELAY_APPLY_WIZARD_CHANGES, ELAY_KB_Core::ELAY_UPDATE_KB_WIZARD_ARTICLE_COLOR_VIEW) ) ) {
			new ELAY_KB_Config_Controller();
			new ELAY_Layout_Sidebar();
			new ELAY_Layout_Sidebar_v2();
			new ELAY_Layout_Grid();
		...
		if ( in_array($action, array(ELAY_KB_Core::ELAY_UPDATE_KB_WIZARD_ORDER_VIEW, ELAY_KB_Core::ELAY_UPDATE_KB_WIZARD_PREVIEW) ) ) {
			new ELAY_KB_Config_Controller();
			new ELAY_Layout_Sidebar();
			new ELAY_Layout_Sidebar_v2();
			new ELAY_Layout_Grid();
	...
	private function setup_backend_classes() {
		...
		// enqueue script for KB Config page or if GT/Elementor editor
		if ( $request_page == ELAY_KB_Core::ELAY_KB_CONFIGURATION_PAGE ) {
			add_action( 'admin_enqueue_scripts', 'elay_kb_config_load_public_css' );
		}
		...
		$classes = array(
			...
			array( 'ELAY_KB_Config_Layouts', array('backend'), array('edit.php', ELAY_KB_Core::ELAY_KB_CONFIGURATION_PAGE), array(''), true ),
			array( 'ELAY_Layout_Sidebar', array('backend'), array('edit.php', ELAY_KB_Core::ELAY_KB_CONFIGURATION_PAGE), array(''), true ),
			...
			array( 'ELAY_Settings_Page', array('backend'), array(ELAY_KB_Core::ELAY_KB_DEBUG_PAGE), array(''), true ),
