...
class FrmAPIFieldsController extends WP_REST_Controller {
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( 'edit' === $request['context'] && ! current_user_can( 'frm_edit_forms' ) ) {
			return new WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit form fields' ), array( 'status' => 403 ) );
		} elseif ( 'view' === $request['context'] && ! current_user_can( 'frm_view_forms' ) ) {
			return new WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to view form fields' ), array( 'status' => 403 ) );
		}
