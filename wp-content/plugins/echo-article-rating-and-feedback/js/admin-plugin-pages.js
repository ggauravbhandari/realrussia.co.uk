jQuery(document).ready(function($) {
    ...
    license_form.on('click', '#eprf_save_btn', function (e) {
        ...
        let postData = {
            ...
            _wpnonce_eprf_license_key: $('#_wpnonce_eprf_license_key').val(),
            command: 'save'
        };
	...
	$(document.body).on('click', '#resetArticleRating', function(){
		...
		if ( typeof eprf_article_rating_data_delete._wpnonce_edit_article == 'undefined' ) {
			//should never run, added just in case
			alert( 'Missing EPRF rating data.' );
			return true;
		}
		
		if (confirm(eprf_article_rating_data_delete.delete_confirm)) {
			// find article id
			...
			let postData = {
				...
				_wpnonce_edit_article: eprf_article_rating_data_delete._wpnonce_edit_article,
