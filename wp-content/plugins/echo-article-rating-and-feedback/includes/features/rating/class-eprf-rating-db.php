...
class EPRF_Rating_DB extends EPRF_DB  {
	 ...
	 */
	public function has_article_user_IP( $kb_id, $post_id ) {
		...
		if ( ! empty($user) ) {
			$result = $this->get_a_row_by_where_clause( array('user_id' => $user->ID, 'post_id' => $post_id) );
			if ( is_wp_error( $result) ) {
				EPRF_Logging::add_log("Cannot get article: ", $post_id);
				return $result;
			}
		...
		// if user NOT logged in then check by IP
		$user_ip = EPRF_Core_Utilities::get_ip_address();
		$result = $this->get_a_row_by_where_clause( array('user_ip' => $user_ip, 'post_id' => $post_id) );
		if ( is_wp_error( $result) ) {
			EPRF_Logging::add_log("Cannot get article: ", $post_id);
			return $result;
		}
