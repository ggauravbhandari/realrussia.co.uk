...
class EPRF_Analytics_View {
		...
		return array_merge(
			...
			array(
				...
				array(
					...
					// Top Panel Item
					'label_text' => __( 'Rating Data', 'echo-advanced-search' ),
	...
	private function pie_chart_rating_data_box( $title, $data, $id, $empty_message='' ) {   ?>
		...
		<section class="eprf-pie-chart-container" id="<?php echo $id; ?>">
			...
			<div class="eprf-pie-chart-header">
				...
				<i class="eprf-pie-chart-cog epkbfa epkbfa-cog" aria-hidden="true"></i>
			</div>
					...
					<ul class="eprf-pie-data-list">			<?php
						if ( empty($data) ) {
							...
							foreach ( $data as $word ) {    ?>
								<li class="<?php echo $i++ <= 10 ? 'eprf-first-10' : 'eprf-after-10'; ?>">
	...
	/* public function handle_rating_analytics() {
		...
		if ( empty( $_REQUEST['_wpnonce_eprf_rating_analytics'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_eprf_rating_analytics'], '_wpnonce_eprf_rating_analytics' ) ) {
