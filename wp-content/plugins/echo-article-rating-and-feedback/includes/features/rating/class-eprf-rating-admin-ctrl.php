...
class EPRF_Rating_Admin_Cntrl {
	...
	public function handle_reset_article_feedback() {
		...
		if ( empty( $_REQUEST['_wpnonce_edit_article'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_edit_article'], '_wpnonce_edit_article' ) ) {
		...
		if ( empty($article_id) || empty($kb_id) ) {
			wp_die( json_encode( array( 'status' => 'error', 'message' => __('Could not reset (0)', 'echo-article-rating-and-feedback') ) ) );
		}

		$db_handler = new EPRF_Rating_DB();
		$isDeleted = $db_handler->delete_article_rating($kb_id, $article_id);
		if ( ! $isDeleted ) {
			wp_die( json_encode( array( 'status' => 'error', 'message' => __('Could not reset (1)', 'echo-article-rating-and-feedback') ) ) );
		}
