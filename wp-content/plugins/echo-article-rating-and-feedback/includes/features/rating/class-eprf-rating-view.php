...
class EPRF_Rating_View {
	...
	public function show_rating_statistics( $data ) {
		...
		if ( $add_on_config[ 'rating_switch_off' ] == 'on' ) {
			return;
		}
		...
		<div class="eprf-article-meta__star-rating" <?php echo $inline_style; ?>>
			...
			<div class="eprf-article-meta__star-rating__stars-stats">
				...
				<div class="eprf-article-meta__statistics">
					<h6><?php echo $rating[ 'average' ] . ' ' . $add_on_config[ 'rating_out_of_stars_text' ]; ?></h6>
					<table> <?php
	...
	private function show_rating_element_and_feedback_form( $data, $isTopHook = false ) {
		...
		$add_on_config = eprf_get_instance()->kb_config_obj->get_kb_config_or_default( $kb_id );
		$add_on_config = array_merge( $add_on_config, $kb_config );
		if ( $add_on_config[ 'rating_switch_off' ] == 'on' || $add_on_config[ 'article_content_enable_rating_element' ] == 'off' ) {
			return;
		}
	...
	public function show_rating_element( $kb_id, $kb_config, $article_id, $did_vote, $isConfiguredTop ) {
		if ( $kb_config[ 'rating_switch_off' ] == 'on' ) {
			return;
		}
	...
	<div
		...
		<div class="eprf-like-dislike-module__text" <?php echo $rating_style; ?>>
			<?php echo $pagenow == 'post-new.php' || ( $pagenow == 'edit.php' && isset( $_GET[ 'page' ] ) && $_GET[ 'page' ] !== 'epkb-kb-configuration' ) || ( $pagenow == 'edit.php' && !isset( $_GET[ 'page' ] ) ) || $pagenow == 'post.php' ? '' : $rating_text; ?>
		</div>
