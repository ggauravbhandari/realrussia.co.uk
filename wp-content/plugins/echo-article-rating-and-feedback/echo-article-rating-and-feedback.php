...
final class Echo_Article_Rating_And_Feedback {
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action,
				array(EPRF_KB_Core::EPRF_CHANGE_ARTICLE_PAGE_CONFIG_AJAX, EPRF_KB_Core::EPRF_CHANGE_MAIN_PAGE_CONFIG_AJAX,
			          EPRF_KB_Core::EPRF_CHANGE_ONE_PARAM_AJAX, EPRF_KB_Core::EPRF_CHANGE_ARTICLE_CATEGORY_SEQUENCE) ) ) {
			new EPRF_KB_Config_Controller();
			new EPRF_KB_Config_Page();
			new EPRF_Rating_View();
			return;
		}

		if ( in_array($action, array(EPRF_KB_Core::EPRF_SAVE_KB_CONFIG_CHANGES) )  ) {
			new EPRF_KB_Config_Controller();
			return;
		}
		...
		if ( in_array($action, array(EPRF_KB_Core::EPRF_APPLY_WIZARD_CHANGES, EPRF_KB_Core::EPRF_UPDATE_KB_WIZARD_ARTICLE_COLOR_VIEW)) ) {
			new EPRF_KB_Config_Controller();
			new EPRF_Rating_View();
		...
		if ( in_array($action, array(EPRF_KB_Core::EPRF_UPDATE_KB_WIZARD_ORDER_VIEW, EPRF_KB_Core::EPRF_UPDATE_KB_WIZARD_PREVIEW)) ) {
			new EPRF_KB_Config_Controller();
			new EPRF_Rating_View();
			return;
		}
		
		if ( in_array( $action, array( EPRF_KB_Core::EPRF_APPLY_SETUP_WIZARD_CHANGES, EPRF_KB_Core::EPRF_UPDATE_KB_EDITOR)) ) {
			new EPRF_KB_Config_Controller();
		}
	}
	...
	private function setup_backend_classes() {
		...
		$classes = array(
			...
			array( 'EPRF_KB_Config_Page', array('backend'), array('edit.php', EPRF_KB_Core::EPRF_KB_CONFIGURATION_PAGE), array(''), true ),
