...
class WIDG_Category_Articles_Shortcode {
	...
	public function output_shortcode( $attributes ) {
		...
		$title = $title == 'empty' ? '' :
				('<h4>' . ( empty($title) ? esc_html__( 'Category Articles', 'echo-widgets' ) : esc_html( $title ) ) . '</h4>');
		...
		$include_children = empty($attributes['include_children']) || $attributes['include_children'] != 'on' ? false : true;
