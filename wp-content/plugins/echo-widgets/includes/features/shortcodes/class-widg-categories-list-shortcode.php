...
class WIDG_Categories_List_Shortcode {
	...
	public function output_shortcode( $attributes ) {
        ...
        $title = '<h4>' . ( empty($title) ? esc_html__( 'Categories', 'echo-widgets' ) : esc_html( $title ) ) . '</h4>';
		...
		$terms = $this->execute_search( $kb_id, $filter, $this->retrieve_category_ids( $attributes ) );
	    ...
	    echo '<div class="widg-shortcode-categories-container">';
		...
		echo '</div>'; //widg-shortcode-categories-container
