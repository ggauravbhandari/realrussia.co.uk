...
class WIDG_KB_Search {
	...
	public function search_kb() {
		...
		if ( is_wp_error( $kb_id ) ) {
			wp_die( json_encode( array( 'status' => 'success', 'search_result' => esc_html__( 'Error occurred. Please try again later.', 'echo-widgets' ) ) ) );
		}
		...
		if ( empty($search_terms) ) {
            wp_die( json_encode( array( 'status' => 'success', 'search_result' => esc_html( $min_search_word_size_msg ) ) ) );
		}
		...
		if ( empty($result) ) {
			...
			$search_result = '<div class="widg-search-results-content">';
            $search_result .= '<div>' . esc_html( $search_results_msg ) . ': <strong>' . $search_terms . '</strong></div>';
			$search_result .= '<ul>';
	...
	private function execute_search( $search_terms, $kb_id ) {
		...
		$nof_articles = widg_get_instance()->kb_config_obj->get_value( $kb_id, 'widg_search_results_limit', 8 );
