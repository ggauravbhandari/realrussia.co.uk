...
class WIDG_KB_Config_DB {
	...
	function get_kb_configs( $skip_check=false ) {
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( count($config) < 100 ) {
				WIDG_Logging::add_log("Found KB configuration is incomplete", count($config));
			}
			...
			if ( WIDG_Utilities::is_wpml_enabled_addon( $kb_id ) ) {
	...
	public function get_kb_config( $kb_id, $default_on_error=true ) {
		...
		if ( empty($config) || ! is_array($config) ) {
			...
			return ( $default_on_error ? WIDG_KB_Config_Specs::get_default_kb_config() :
				new WP_Error('get_kb_config', 'Did not find KB Config. Try to deactivate and reactivate KB plugin to see if this fixes the issue. ' . WIDG_Utilities::contact_us_for_support()) );
		}

		if ( count($config) < 100 ) {
			WIDG_Logging::add_log("Found WIDG configuration is incomplete", count($config));
		}
	...
	public function get_value( $kb_id, $setting_name, $default = '' ) {
		...
		if ( is_wp_error( $kb_config ) ) {
			WIDG_Logging::add_log( "Could not retrieve KB configuration (15). Settings name: ", $setting_name, $kb_config );
			return $default;
		}
	...
	public function update_kb_configuration( $kb_id, array $config, $upsert=true ) {
		...
		if ( is_wp_error($sanitized_config) ) {
			WIDG_Logging::add_log( 'Could not update WIDG configuration', $kb_id, $sanitized_config );
	...
	private function save_kb_config( array $config, $kb_id, $upsert=true ) {
		...
		if ( $result === false ) {
			WIDG_Logging::add_log( 'Failed to update kb config for kb_id', $kb_id );
	...
	public function kb_config_filter( $kb_config ) {
		$kb_config = array_merge( $kb_config, self::get_kb_config_or_default( $kb_config['id'] ) );
		return $kb_config;
	}
}