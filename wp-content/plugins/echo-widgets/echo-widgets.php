...
final class Echo_Widgets {
	...
	public static function instance( $dormant=false ) {
		...
		if ( isset( self::$instance ) || self::$instance instanceof Echo_Widgets  ) {
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action,
				array(WIDG_KB_Core::WIDG_CHANGE_ARTICLE_PAGE_CONFIG_AJAX, WIDG_KB_Core::WIDG_CHANGE_MAIN_PAGE_CONFIG_AJAX) ) ) {
			new WIDG_KB_Config_Page();
			return;
		}
		if ( in_array($action, array(WIDG_KB_Core::WIDG_SAVE_KB_CONFIG_CHANGES) )  ) {
		...
		if ( in_array($action, array(WIDG_KB_Core::WIDG_APPLY_WIZARD_CHANGES, WIDG_KB_Core::WIDG_UPDATE_KB_WIZARD_ARTICLE_COLOR_VIEW)) ) {
			new WIDG_Shortcodes();
			new WIDG_KB_Config_Controller();
		...
		if ( in_array($action, array(WIDG_KB_Core::WIDG_UPDATE_KB_WIZARD_ORDER_VIEW, WIDG_KB_Core::WIDG_UPDATE_KB_WIZARD_PREVIEW)) ) {
			new WIDG_Shortcodes();
			new WIDG_KB_Config_Controller();
		...
		if ( in_array( $action, array( WIDG_KB_Core::WIDG_APPLY_SETUP_WIZARD_CHANGES, WIDG_KB_Core::WIDG_UPDATE_KB_EDITOR)) ) {
			new WIDG_KB_Config_Controller();
		}
	}
	...
	private function setup_backend_classes() {
		...
		if ( $pagenow == 'post.php' ) {
			...
			$post = WIDG_Utilities::get_kb_post_secure( $post_id );
		...
		$classes = array(
            ...
            array( 'WIDG_KB_Config_Page', array('backend'), array('edit.php', WIDG_KB_Core::WIDG_KB_CONFIGURATION_PAGE), array(''), true ),
