...
final class Echo_Advanced_Search {
	...
	public static $version = '2.24.0';
	public static $plugin_dir;
	public static $plugin_url;
	public static $plugin_file = __FILE__;
	public static $needs_min_core_version = '4.2.0';
	...
	public static function instance( $dormant=false ) {
		...
		if ( isset( self::$instance ) || self::$instance instanceof Echo_Advanced_Search  ) {
	...
	private function handle_action_request( $action ) {
		if ( in_array($action, array('epkb_load_editor', 'eckb_apply_editor_changes', 'eckb_editor_get_themes_list', 'eckb_theme_wizard_get_themes_v2', ASEA_KB_Core::ASEA_APPLY_SETUP_WIZARD_CHANGES)) ) {
			ASEA_KB_Editor_Config::register_editor_hooks();
			return;
		}
	}
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action, array(ASEA_KB_Core::ASEA_APPLY_SETUP_WIZARD_CHANGES) ) ) {
			new ASEA_KB_Config_Controller();
			ASEA_KB_Wizard::register_all_wizard_hooks();
			return;
		}

		if ( in_array($action, array(ASEA_KB_Core::ASEA_CHANGE_MAIN_PAGE_CONFIG_AJAX, ASEA_KB_Core::ASEA_CHANGE_ARTICLE_PAGE_CONFIG_AJAX)) ) {
			new ASEA_KB_Config_Page();
			new ASEA_Search_Box_View();
			new ASEA_KB_Config_Styles();
			new ASEA_KB_Config_Controller();
			return;
		}
		...
		if ( in_array($action,
			array(ASEA_KB_Core::ASEA_CHANGE_ARTICLE_PAGE_CONFIG_AJAX, ASEA_KB_Core::ASEA_CHANGE_MAIN_PAGE_CONFIG_AJAX, 'eckb_apply_editor_changes',
				  ASEA_KB_Core::ASEA_SAVE_KB_CONFIG_CHANGES, ASEA_KB_Core::ASEA_CHANGE_ONE_PARAM_AJAX, ASEA_KB_Core::ASEA_CHANGE_ARTICLE_CATEGORY_SEQUENCE) ) ) {
			new ASEA_KB_Config_Controller();
			new ASEA_Search_Box_View();
			return;
		}
		
		if ( in_array($action, array(ASEA_KB_Core::ASEA_APPLY_WIZARD_CHANGES, ASEA_KB_Core::ASEA_UPDATE_KB_WIZARD_ARTICLE_COLOR_VIEW, ASEA_KB_Core::ASEA_UPDATE_KB_WIZARD_PREVIEW)) ) {
			new ASEA_KB_Config_Controller();
			new ASEA_Search_Box_View();
			ASEA_KB_Wizard::register_all_wizard_hooks();
			return;
		...
		if ( in_array($action, array(ASEA_KB_Core::ASEA_UPDATE_KB_WIZARD_ORDER_VIEW, ASEA_KB_Core::ASEA_UPDATE_KB_WIZARD_PREVIEW)) ) {
			new ASEA_KB_Config_Controller();
			new ASEA_Search_Box_View();
			return;
		}
	...
	private function setup_backend_classes() {
		...
		$classes = array(
			...
			array( 'ASEA_KB_Config_Page', array('backend'), array(ASEA_KB_Core::ASEA_KB_CONFIGURATION_PAGE), array(''), true ),
			...
			array( 'ASEA_Settings_Page', array('backend'), array(ASEA_KB_Core::ASEA_KB_DEBUG_PAGE), array(''), true ),
