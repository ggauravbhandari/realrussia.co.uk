...
$search_terms_new = ASEA_Utilities::get( _x( 'kb-search', 'search query parameter in URL', 'echo-advanced-search' ) );
...
function asea_display_search_list( $kb_config, $found_posts, $search_terms, $page_num, $search_total, $user_category_ids ) {     ?>
	...
	<footer class="asea-search-results-footer">				<?php
		...
		if ( $page_num > 1 ) {
			echo '<a class="asea-paginate-button asea-previous-button" href="' .  esc_url( '?' . _x( 'kb-search', 'search query parameter in URL', 'echo-advanced-search' ) . '=' . urlencode($search_terms) . $user_category_ids_url . ($page_num == 2 ? '' : '&' . _x('pg', 'abbreviation for the word: page', 'echo-advanced-search') . '=' . ($page_num - 1)) ) . '">' . __('Previous Page', 'echo-advanced-search') . '</a>';
		}

		// Next Page button
		if ( $search_total > ASEA_Search_Box_cntrl::get_search_results_page_size( $kb_config['id'] ) * $page_num ) {
			echo '<a class="asea-paginate-button asea-next-button" href="' .  esc_url( '?' . _x( 'kb-search', 'search query parameter in URL', 'echo-advanced-search' ) . '=' . urlencode($search_terms) . $user_category_ids_url . '&' . _x('pg', 'abbreviation for the word: page', 'echo-advanced-search') . '=' . ($page_num + 1) ) . '">' . __('Next Page', 'echo-advanced-search') . '</a>';
		}				?>
	</footer>       <?php
}
