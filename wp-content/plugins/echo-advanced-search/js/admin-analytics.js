jQuery(document).ready(function($) {
	...
	function display_asea_pie_chart( container_id ){
		...
		if( $( '.asea-analytics-container' ).length > 0 ){
			...
			$first_ten_count = [];
			$first_ten_words = [];
			total = $( '#'+container_id+ ' .asea-pie-data-list .asea-first-10' ).length;
	...
	//Show Hide Pie chart data
	if( $( '.asea-analytics-container' ).length > 0 ){

		$( '.asea-pie-chart-cog' ).on( 'click', function(){

			if( $(this).hasClass( 'ep_font_icon_arrow_carrot_down' ) ){

				$(this).removeClass( 'ep_font_icon_arrow_carrot_down' );
				$(this).addClass( 'ep_font_icon_arrow_carrot_up' );

			}else if( $(this).hasClass( 'ep_font_icon_arrow_carrot_up' ) ){

				$(this).removeClass( 'ep_font_icon_arrow_carrot_up' );
				$(this).addClass( 'ep_font_icon_arrow_carrot_down' );
	...
	if ( $( '.asea-reset-container' ).length > 0 ){
		...
		$( '.asea-reset-analytics' ).on( 'click',  function(){
			...
			$( '.asea-dialog-box-form' ).toggleClass( 'asea-dialog-box-form--active' );
			return false;
		});
