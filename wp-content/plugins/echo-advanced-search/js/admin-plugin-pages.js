jQuery(document).ready(function($) {
	...
	if ( $('.asea-config-container').length ) {
		...
		$('.asea-config-container').on('click', '#asea_search_config_save', function (e) {
			...
			{
				response = ( response ? response : '' );
				if ( response.status || typeof response.message === 'undefined' || typeof response.kb_info_panel_output === 'undefined' ) {
					//noinspection JSUnresolvedVariable,JSUnusedAssignment
					msg = asea_admin_bottom_notification('', response.message, response.status);
					return;
				}
