			...
			if ( is_wp_error($result) ) {
                ...
                if ( ( empty($field_spec['internal']) && count($errors) > 0 ) || ( defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ) ||
                 ! in_array( $field_spec['type'], array(self::CHECKBOX, self::SELECTION, self::CHECKBOXES_MULTI_SELECT, self::CHECKBOXES_MULTI_SELECT_NOT, self::TRUE_FALSE, self::ENUMERATION) )) {
                    ...
                    $error_page_name = "";
                    if ( strpos($field_spec['name'], "_mp_") !== false ) {
                        $error_page_name = __( 'Main Page: ', 'echo-knowledge-base' );
                    } else if ( strpos($field_spec['name'], "_ap_") !== false ) {
                        $error_page_name = __( 'Article Page: ', 'echo-knowledge-base' );
                    }

                    $lang = '<strong style="color:#d5ff8b">' . $field_spec['label'] . '</strong>';
                    $errors[] = '<div style="padding: 20px 0 20px 0;">'. $error_page_name . sprintf( __( 'Please change value of the %s field.', 'echo-knowledge-base' ), $lang ) . ' ' . $result->get_error_message() . '</div>';
