 ...
 */
	...
	public function display_search_title( $kb_config ) {
		...
		$title_style .= 'padding-bottom:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_title_padding_bottom') . 'px;';
		...
		if ( $search_title_toggle == 'on' ) { ?>
			<<?php echo $title_tag; ?> id="asea-search-title"<?php echo $title_style; ?>> <?php echo wp_kses_post( $search_title ); ?></<?php echo $title_tag; ?>>	<?php
		}
	}
	...
	public function display_paragraph_description_below_title( $kb_config ) {
		...
		$desc_style .= 'padding-top:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_description_below_title_padding_top') . 'px;';
		$desc_style .= 'padding-bottom:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_description_below_title_padding_bottom') . 'px;';
		...
		if ( $below_title_text_toggle == 'on' ) {     ?>
			<p id="asea-search-description-1" <?php echo $desc_style; ?>> <?php echo wp_kses_post( $below_title_text ); ?></p>  <?php
		}
	}
	...
	public function display_search_input_and_results_box( $kb_config ) {
		...
		$search_box_style .= 'border-width:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_input_border_width' ) . 'px;';
		...
		$search_box_style .= 'border-color:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_text_input_border_color' ) . ';';
		$search_box_style .= 'background-color:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_text_input_background_color' ) . ';';
		...
		$search_box_style .= 'padding-left:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_input_box_padding_left' ) . 'px;';
		$search_box_style .= 'padding-right:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_input_box_padding_right' ) . 'px;';
		...
		// Input Box Styles --------------------------------------------------------------------------------------------/
		$input_style  = 'style="';
		$input_style .= 'padding-top:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_input_box_padding_top' ) . 'px;';
		$input_style .= 'padding-bottom:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_input_box_padding_bottom' ) . 'px;';
		...
		if ( ( ! is_rtl() && $search_filter == 'on' ) || $search_icon_placement == 'right' || $loading_icon_placement == 'right' ) {
			$input_style .= 'padding-right:15px;';
		}
		...
		<div id="asea-doc-search-box-container">
			...
			<form id="asea_search_form" <?php echo $form_style . ' ' . $class1; ?> method="get" action="" role="search" aria-label="Search">
				...
				<!----- Search Box ------>
				<div class="asea-search-box" <?php echo $search_box_style; ?>><?php
					...
					<input type="search" <?php echo $input_style; ?> id="asea_advanced_search_terms" name="asea_search_terms" value=""
					...
					if ( ! empty($kb_config['seq_id']) ) { ?>
						<input type="hidden" id="asea_seq_id" value="<?php echo $kb_config['seq_id']; ?>"/>					<?php 
					}
				...
				<div class="asea-search-filter-container <?php echo $category_level; ?>" <?php echo $search_dropdown_style; ?>>
					...
					<fieldset>
						<legend><?php echo $filter_by_title ?></legend>
						<span id="asea-search-filter-clear-results"><?php echo $clear_results_title ?></span>						<?php
						...
						$unfiltered_categories = ASEA_Core_Utilities::get_kb_categories_unfiltered( $kb_config['id'] );
							...
							foreach( $unfiltered_categories as $top_category ) {
								...
								if ( in_array($top_category->term_id, $top_filtered_categories) || ! empty($child_categories) ) { ?>
									...
									<div class="asea-filter-option">
										<label>
											<input class="asea-filter-option-input" name="cat-<?php echo $top_category->term_id; ?>" type="checkbox" value="<?php echo $top_category->term_id; ?>" autocomplete="off">
											<span class="asea-filter-option-label"><?php echo $top_category->name; ?></span>
										</label>
									</div>                                <?php
								}
								     ...
								     <ul class="children" style="margin-left:20px;list-style:none;display:none;"> <?php
                                        foreach( $child_categories as $child_category ) { ?>
                                            <li>
                                                <div class="asea-filter-option">
                                                    <label>
                                                        <input class="asea-filter-option-input" name="cat-<?php echo $child_category->term_id; ?>" type="checkbox" value="<?php echo $child_category->term_id; ?>" autocomplete="off">
	...
	public function asea_search_filter_icon( $kb_config ) {
		...
		echo '<div class="asea-search-filter-icon-container" ' . $filter_icon_style . '>';
			...
			if ( ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_filter_indicator_text' ) ) {
				echo '<span class="asea-search-filter-text">' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_filter_indicator_text' ) . '</span>';
			}
	...
	public function asea_search_container_style_tags( $kb_config, $prefix = '' ) {
		echo '<style>';
			echo $prefix . ' #asea-doc-search-container #asea-section-1 a { color:' . ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_link_font_color') . ' }';
		echo '</style>';
	}
	...
	public function eckb_doc_search_container_classes( $kb_config ) {
		echo ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_box_visibility' ) . ' ';
	}
	...
	public function search_toggle( $kb_config ) {
		if ( ASEA_Core_Utilities::get_search_kb_config( $kb_config, 'advanced_search_*_box_visibility' ) === 'asea-visibility-search-form-3' ) {    	?>
