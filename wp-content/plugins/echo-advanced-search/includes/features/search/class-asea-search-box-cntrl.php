	...
	private function get_search_results_dropdown( $kb_id, $first_kb_config, $current_kb_config, $search_terms, $found_posts, $search_total, $user_category_ids ) {
		...
		foreach( $found_posts as $post ) {
			...
			if ( ASEA_Core_Utilities::get_search_kb_config( $first_kb_config, 'advanced_search_*_show_top_category' ) == 'on' ) {
				...
				$article_categories = ASEA_Core_Utilities::get_article_categories_unfiltered( $kb_id, $post->ID );
			...
			$search_result .=
				'<li>' .
					...
					'<span class="eckb-article-title" ' . $title_style . '>' .
						'<span class="eckb-article-title-icon epkbfa ' . esc_attr($article_title_icon) . '" ' . $icon_style . '></span>' .
		...
		if ( $search_total > count($found_posts) ) {
			...
			if ( ! empty($permalink) ) {
				...
				$searchpage_url = add_query_arg( array( _x('kb-search', 'search query parameter in URL', 'echo-advanced-search') => urlencode($search_terms) ), $permalink ) . $user_category_ids_url;
			}
