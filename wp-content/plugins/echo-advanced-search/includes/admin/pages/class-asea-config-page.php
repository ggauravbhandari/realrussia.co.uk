...
class ASEA_Config_Page {
	...
	public function asea_search_config_save() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_asea_config'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_asea_config'], '_wpnonce_asea_config' ) ) {
			wp_die( json_encode( array( 'status' => 'error', 'message' => esc_html__( 'You do not have permission to edit this knowledge base', 'echo-knowledge-base' ) ) ) );
			return;
		}
		...
		if ( is_wp_error( $kb_id ) ) {
			wp_die( json_encode( array( 'status' => 'error', 'message' => esc_html__( 'Error occurred. Please try again later.', 'echo-advanced-search' ) ) ) );
		}
		...
		/* if ( $action == 'asea_synonyms_update' ) {
		   ...
		   if ( ! empty($value) && count(explode(",", $value)) > 20 ) {
				wp_die(json_encode(array('status' => 'error', 'message' => esc_html__('Maximum 20 keywords allowed', 'echo-advanced-search') . '(46)')));
		   }
	...
	private function update_asea_config( $kb_id, $key, $value, $success_msg ) {
	   ...
	   if ( is_wp_error($result) ) {
		   wp_die(json_encode(array('status' => 'error', 'message' => esc_html__( 'Error occurred. Please try again later.', 'echo-advanced-search') . '(48) - ' . $key)));
	   }

	   wp_die(json_encode(array('status' => 'success', 'message' => esc_html( $success_msg ) )));
	}
	...
	public function asea_search_audit_clear() {  // TODO move to controller class
		...
		$kb_id = empty($_POST['asea_kb_id']) ? '' : ASEA_Utilities::sanitize_get_id( $_POST['asea_kb_id'] );
		if ( is_wp_error( $kb_id ) ) {
			wp_die( json_encode( array( 'status' => 'error', 'message' => esc_html__( 'Invalid parameter. Please refresh your page', 'echo-knowledge-base' ) )) );
		}
		...
		wp_die( json_encode( array( 'status' => 'success', 'message' => esc_html__( 'Debug Log was cleared', 'echo-advanced-search' ) )) );
	}
