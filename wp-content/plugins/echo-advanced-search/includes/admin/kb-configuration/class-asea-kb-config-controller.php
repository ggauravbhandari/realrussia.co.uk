...
class ASEA_KB_Config_Controller {
	...
	public function __construct() {
		add_filter( ASEA_KB_Core::ASEA_KB_CONFIG_GET_ADD_ON_INPUT, array( $this, 'get_changed_input' ), 10, 3 );
		add_filter( ASEA_KB_Core::ASEA_KB_ADD_ON_CONFIG_SPECS, array( $this, 'get_add_on_config_specs' ), 10, 1 );
		add_filter( ASEA_KB_Core::ASEA_KB_CONFIG_SAVE_INPUT, array( $this, 'save_kb_config_changes_in_db' ), 10, 4 );
	...
	public function save_kb_config_changes_in_db( $status, $kb_id, $form_fields, $main_page_layout ) {
		...
		$result = asea_get_instance()->kb_config_obj->update_kb_configuration( $kb_id, $add_on_config );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
	...
	private function get_new_kb_config( $form_fields_sanitized, $kb_id ) {
		...
		$orig_config = asea_get_instance()->kb_config_obj->get_kb_config( $kb_id );
		if ( is_wp_error( $orig_config ) ) {
			return $orig_config;
		}

		$field_specs = ASEA_KB_Config_Specs::get_fields_specification();
