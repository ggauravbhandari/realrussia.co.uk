...
class EMKB_Utilities {
	...
	const EMKB_ADMIN_CAPABILITY = 'manage_options';
	 ...
	 * @param string $param
	 */
	public static function show_top_level_admin_msg_and_redirect( $msg_code, $redirect='admin.php', $param='', $param2='' ) {
		$url = empty( $_REQUEST['emkb_redirect'] ) ? admin_url( $redirect ) : $_REQUEST['emkb_redirect'];

		$query = array();
		$query['emkb_admin_notice'] = urlencode($msg_code);
		if ( ! empty($param) ) {
			$query['emkb_notice_param'] = $param;
		}
		
		if ( ! empty($param2) ) {
			$query['emkb_notice_param_2'] = $param2;
		}
	...
	public static function dialog_box_form( $values ) { ?>
		...
		<div id="<?php echo $values[ 'id' ]; ?>" class="emkb-dialog-box-form">

			<!---- Header ---->
			<div class="emkb-dbf__header">
				<h4><?php echo $values['title']; ?></h4>
			</div>

			<!---- Body ---->
			<div class="emkb-dbf__body">				<?php 
				echo empty( $values['body']) ? '' : $values['body']; ?>
			</div>

			<!---- Form ---->
			<form class="emkb-dbf__form">				<?php
				if ( isset($values['form_inputs']) ) {
					foreach ( $values['form_inputs'] as $input ) {
						echo '<div class="emkb-dbf__form__input">' . $input . '</div>';
					}
				}; ?>
			</form>

			<!---- Footer ---->
			<div class="emkb-dbf__footer">

				<div class="emkb-dbf__footer__accept <?php echo isset($values['accept_type']) ? 'emkb-dbf__footer__accept--'.$values['accept_type'] : 'emkb-dbf__footer__accept--success'; ?>">
					<span id="emkb-accept-button" class="emkb-dbf__footer__accept__btn">
						<?php echo $values['accept_label'] ? $values['accept_label'] : __( 'Accept', 'echo-knowledge-base' ); ?>
					</span>
				</div>

				<div class="emkb-dbf__footer__cancel">
					<span class="emkb-dbf__footer__cancel__btn"><?php _e( 'Cancel', 'echo-knowledge-base' ); ?></span>
				</div>

			</div>

			<div class="emkb-dbf__close epkbfa epkbfa-times"></div>

		</div>
		<div class="emkb-dialog-box-form-black-background"></div>		<?php
	}
	...
	public static function retrieve_and_sanitize_form( $form, $all_fields_specs ) {
		...
		foreach( $submitted_fields as $submitted_key => $submitted_value ) {
			...
			if ( ! empty($all_fields_specs[$submitted_key]['type']) && $all_fields_specs[$submitted_key]['type'] == EMKB_Input_Filter::WP_EDITOR ) {
				$sanitized_fields[$submitted_key] = wp_kses_post( $submitted_value );
			} elseif ( ! empty( $all_fields_specs[$submitted_key]['type'] ) && ! empty( $all_fields_specs[$submitted_key]['allowed_tags'] ) && $all_fields_specs[$submitted_key]['type'] == EMKB_Input_Filter::TEXT ) {
    ...
    public static function get_kb_categories_unfiltered( $kb_id, $order_by='name' ) {
        ...
        $kb_category_taxonomy_name = EMKB_KB_Handler::get_category_taxonomy_name( $kb_id );
	...
	public static function get_article_categories_unfiltered( $kb_id, $article_id ) {
		...
		$post_taxonomy_objs = $wpdb->get_results( $wpdb->prepare(
											...
											EMKB_KB_Handler::get_category_taxonomy_name( $kb_id ), $article_id ) );
    ...
    public static function get_kb_category_unfiltered( $kb_id, $kb_category_id ) {
	    $term = get_term_by('id', $kb_category_id, EMKB_KB_Handler::get_category_taxonomy_name( $kb_id ) );
	    if ( empty($term) || ! $term instanceof WP_Term ) {
		    EMKB_Logging::add_log("Category is not KB Category: " . $kb_category_id . "(35)", $kb_id);
		    return false;
	    }
	...
	public static function get_kb_category_by_slug_unfiltered( $kb_id, $kb_category_slug ) {
		$term = get_term_by('slug', $kb_category_slug, EMKB_KB_Handler::get_category_taxonomy_name( $kb_id ) );
		if ( empty($term) || ! $term instanceof WP_Term ) {
			EMKB_Logging::add_log("Category is not KB Category: " . $kb_category_slug . "(34)", $kb_id);
			return false;
		}
    ...
    public static function insert_kb_category( $kb_id, $category_name, $args=array() ) {
	    ...
	    if ( is_wp_error($new_term) ) {
		    EMKB_Logging::add_log( 'Failed to insert category. cat name: ' . $category_name . ', taxonomy: ' . $kb_category_taxonomy_name . ' kb id: ' . $kb_id, $args, $new_term );
		    return null;
	    }
	    if ( ! isset($new_term['term_id']) ) {
		    EMKB_Logging::add_log( 'Failed to insert category. cat name: ' . $category_name . ', taxonomy: ' . $kb_category_taxonomy_name . ' kb id: ' . $kb_id, $args );
		    return null;
	    }
	...
	public static function get_kb_id( $post=null ) {
		...
		if ( ! empty($post) && $post instanceof WP_Post ) {
			$kb_id = EMKB_KB_Handler::get_kb_id_from_post_type( $post->post_type );
		}

		$kb_id = empty($kb_id) || is_wp_error($kb_id) ? ( empty($eckb_kb_id) ? '' : $eckb_kb_id ) : $kb_id;
		if ( empty($kb_id) ) {
			EMKB_Logging::add_log("KB ID not found", $kb_id);
			return null;
		}
	...
	public static function contact_us_for_support() {
		return _x(' Please contact us, click <a href="https://www.echoknowledgebase.com/technical-support/" target="_blank" rel="noopener noreferrer">here</a> for support.', 'Use the same html, change only "here" word inside the link', 'echo-knowledge-base');
	}
}
