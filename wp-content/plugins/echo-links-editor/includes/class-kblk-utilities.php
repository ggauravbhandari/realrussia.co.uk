...
class KBLK_Utilities {
	...
	public static function get_bottom_notice_message_box($message, $title='', $type='success' ) {
		...
		return
			"<div class='eckb-bottom-notice-message'>
				...
				<div class='kblk-close-notice epkbfa epkbfa-window-close'></div>
			</div>";
	}
	...
	public static function retrieve_and_sanitize_form( $form, $all_fields_specs ) {
		...
		foreach( $submitted_fields as $submitted_key => $submitted_value ) {
			...
			if ( ! empty($all_fields_specs[$submitted_key]['type']) && $all_fields_specs[$submitted_key]['type'] == KBLK_Input_Filter::WP_EDITOR ) {
				$sanitized_fields[$submitted_key] = wp_kses_post( $submitted_value );
			} elseif ( ! empty( $all_fields_specs[$submitted_key]['type'] ) && ! empty( $all_fields_specs[$submitted_key]['allowed_tags'] ) && $all_fields_specs[$submitted_key]['type'] == KBLK_Input_Filter::TEXT ) {
	...
	public static function get_wp_option( $option_name, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			KBLK_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Option Name: ' . $option_name );
	...
	public static function get_postmeta( $post_id, $meta_key, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			KBLK_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Meta Key: ' . $meta_key );
			return new WP_Error(__( 'Database failure', 'echo-knowledge-base' ), $wpdb->last_error);
		}
	...
	public static function get_postmeta_multiple( $post_ids, $meta_key, $default, $return_error=false ) {
		...
		if ( ! is_array($post_ids) ) {
			return $return_error ? new WP_Error( 'Invalid Post IDs', self::get_variable_string( $post_ids ) ) : $default;
		}
    ...
    public static function get_kb_categories_unfiltered( $kb_id, $order_by='name' ) {
        ...
        $kb_category_taxonomy_name = KBLK_KB_Handler::get_category_taxonomy_name( $kb_id );
	...
	public static function get_article_categories_unfiltered( $kb_id, $article_id ) {
		...
		$post_taxonomy_objs = $wpdb->get_results( $wpdb->prepare(
											...
											KBLK_KB_Handler::get_category_taxonomy_name( $kb_id ), $article_id ) );
	...
	public static function get_kb_category_by_slug_unfiltered( $kb_id, $kb_category_slug ) {
		$term = get_term_by('slug', $kb_category_slug, KBLK_KB_Handler::get_category_taxonomy_name( $kb_id ) );
		if ( empty($term) || ! $term instanceof WP_Term ) {
			KBLK_Logging::add_log("Category is not KB Category: " . $kb_category_slug . "(34)", $kb_id);
			return false;
		}
    ...
    public static function insert_kb_category( $kb_id, $category_name, $args=array() ) {
	    ...
	    if ( is_wp_error($new_term) ) {
		    KBLK_Logging::add_log( 'Failed to insert category. cat name: ' . $category_name . ', taxonomy: ' . $kb_category_taxonomy_name . ' kb id: ' . $kb_id, $args, $new_term );
		    return null;
	    }
	    if ( ! isset($new_term['term_id']) ) {
		    KBLK_Logging::add_log( 'Failed to insert category. cat name: ' . $category_name . ', taxonomy: ' . $kb_category_taxonomy_name . ' kb id: ' . $kb_id, $args );
		    return null;
	    }
	...
	public static function get_kb_id( $post=null ) {
		...
		if ( ! empty($post) && $post instanceof WP_Post ) {
			$kb_id = KBLK_KB_Handler::get_kb_id_from_post_type( $post->post_type );
		}

		$kb_id = empty($kb_id) || is_wp_error($kb_id) ? ( empty($eckb_kb_id) ? '' : $eckb_kb_id ) : $kb_id;
		if ( empty($kb_id) ) {
			KBLK_Logging::add_log("KB ID not found", $kb_id);
			return null;
		}
