...
class KBLK_KB_Config_DB {
	...
	function get_kb_configs( $skip_check=false ) {
		...
		if ( empty($kb_options) || ! is_array($kb_options) ) {
			KBLK_Logging::add_log("Did not retrieve any kb config. Using defaults", $kb_options);
			$kb_options = array();
		}
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( $config === false ) {
				KBLK_Logging::add_log("Could not unserialize configuration");
				continue;
			}
			...
			if ( count($config) < 100 ) {
				KBLK_Logging::add_log("Found KB configuration is incomplete", count($config));
			}
			...
			$is_wpml_on = KBLK_KB_Core::get_value( $kb_id, 'wpml_is_enabled', true );
			if ( $is_wpml_on == 'on' ) {
	...
	public function get_kb_ids() {
		...
		if ( empty($kb_option_names) || ! is_array($kb_option_names) ) {
			KBLK_Logging::add_log("Did not retrieve any kb config. Using defaults", $kb_option_names);
		...
		foreach ( $kb_option_names as $kb_option_name ) {
			...
			$kb_id = KBLK_Utilities::sanitize_int( $kb_id, KBLK_KB_Config_DB::DEFAULT_KB_ID );
			$kb_ids[$kb_id] = $kb_id;
		}

		// at least include default KB ID
		if ( empty($kb_ids) || ! isset($kb_ids[KBLK_KB_Config_DB::DEFAULT_KB_ID]) ) {
			$kb_ids[KBLK_KB_Config_DB::DEFAULT_KB_ID] = KBLK_KB_Config_DB::DEFAULT_KB_ID;
		}
	...
	public function get_kb_config( $kb_id, $default_on_error=true ) {
		...
		if ( empty($config) || ! is_array($config) ) {
			KBLK_Logging::add_log("Did not find KBLK configuration (DB23).", $kb_id);
			return ( $default_on_error ? KBLK_KB_Config_Specs::get_default_kb_config() : new WP_Error('get_kb_config', 'Did not find KB Config') );
		}

		if ( count($config) < 100 ) {
			KBLK_Logging::add_log("Found KBLK configuration is incomplete", count($config));
		}
	...
	public function get_value( $kb_id, $setting_name, $default = '' ) {
		...
		if ( is_wp_error( $kb_config ) ) {
			KBLK_Logging::add_log( "Could not retrieve KB configuration (15). Settings name: ", $setting_name, $kb_config );
			return $default;
		}
	...
	public function update_kb_configuration( $kb_id, array $config, $upsert=true ) {
		...
		if ( is_wp_error($sanitized_config) ) {
			KBLK_Logging::add_log( 'Could not update KBLK configuration', $kb_id, $sanitized_config );
	...
	private function save_kb_config( array $config, $kb_id, $upsert=true ) {
		...
		if ( $result === false ) {
			KBLK_Logging::add_log( 'Failed to update kb config for kb_id', $kb_id );
