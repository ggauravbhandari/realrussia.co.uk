...
final class Echo_Links_Editor {
	...
	public static $needs_min_core_version = '3.4.0';
	...
	public static function instance( $dormant=false ) {
		...
		if ( isset( self::$instance ) || self::$instance instanceof Echo_Links_Editor  ) {
	...
	private function setup_backend_classes() {
		...
		if ( $pagenow == 'post.php' ) {
			...
			$post = KBLK_Utilities::get_kb_post_secure( $post_id, false );
		...
		$classes = array(
			...
			array( 'KBLK_Settings_Page', array('backend'), array(KBLK_KB_Core::KBLK_KB_DEBUG_PAGE), array(''), true ),
        );
...
function kblk_display_version_error() {
	...
	if ( $plugins_check == 'core_missing' ) {
		echo '<div class="error"><p>KB - Links Editor requires <a href="https://wordpress.org/plugins/echo-knowledge-base/"
	...
	if ( $plugins_check == 'core_too_old' ) {
		echo '<div class="error">Links Editor plugin requires at least ' . Echo_Links_Editor::$needs_min_core_version .
		...
		echo '<div class="error">' . $kb_plugin_name . ' plugin requires the most recent version of the Links Editor plugin. The old version of Links Editor stays
			<strong>disabled</strong> until updated. <a href="' . $add_on_url .'" target="_blank">Check its status here.</a></p></div>';
	} else {
	    // should not happen
	    echo '<div class="error">Links Editor: unknown add-on version conflict.</div>';
    }
}
