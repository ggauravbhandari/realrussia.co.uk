...

{
    ...
    function initializiation()
    {
        ...
        if (!empty($this->tourId)) {
            ...
            $this->availDepartureDate = $this->getDepartureDates($this->tourId, $this->month);

            $this->tourLastDay = $this->getTourLastDay($this->tourId);

            $this->departures = $this->getTourDepartures($this->tourId);

            
            $this->firstAvailableDepartureDate = $this->getFirstAvailableDepartureDate($this->tourId, $this->month);
                ...
                WHERE 
                  ...
                  tblRoomCostRanges.roomID = ' . $rowA31['roomID'] . ' and tblRoomCostRanges.dateFrom <= "' . $dayList[$i] . '" and "' . $dayList[$i] . '"<= tblRoomCostRanges.dateTo limit 1';
                        ...
                        foreach ($totalCost as $k => $val) {                            
                            $temp = $this->getPricing($k, $this->currency, $val, false);                            
                            $price = $price + $temp;                            
                        }                        
                        ...
                        foreach ($nonChangableCost as $k => $val) {
                            $temp = $this->getPricing($k, $this->currency, $val, true);                        
                            $price = $price + $temp;
                        }
                ...
                if (count($this->allDiffCostArray) == 0) {
                    ...
                    echo json_encode(
                        array(
                            ...
                            'endingDate' => $this->today, 
                            'departures' => $this->departures,
                            'firstAvailableDepartureDate' => $this->firstAvailableDepartureDate
                        )
                    );
                    exit();
    ...
    function getDepartureDates($tourId, $month)
    {
        ...
        if ($this->lowestPrice) {
            ...
            $sql = "SELECT cheapestDate FROM tblTourData where tourID = $tourId";
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_row()) {
                    ...
                    array_push($departureDate, $row[0]);
            ...
            while ($row = $result->fetch_row()) {
                ...
                switch ($row[0]) {
                    ...
                    case 74:
                        ...
                        $sql = "SELECT dStartDate FROM tblDepartures where tourID = $tourId and month(dStartDate)={$month} and year(dStartDate) = {$this->year} ";
                    ...
                    case 75:
                        ...
                        $tempDate = $this->year . '-' . $this->month . '-01';
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $startDateStr = $tempDate;
                            $row['dStartDate'] = $tempDate;

                            
                            $firstDateOfMonth = strtotime($this->year . '-' . $this->month . '-01');
                            $lastDateOfMonth = strtotime("-1 day", strtotime("+1 month", $firstDateOfMonth));

                            $endDate = $lastDateOfMonth;
                            if (strtotime($row['dEndDate']) < $endDate)
                            {
                                $endDate = strtotime($row['dEndDate']);
                            }

                            $endDateStr = date("Y-m-d", $endDate);

                            $departureDate = $this->getDatesFromRange($startDateStr, $endDateStr);                      
    ...
    function getFirstAvailableDepartureDate($tourId, $month)
    {
            ...
            while ($row = $result->fetch_row()) {
                ...
                switch ($row[0]) {
                    ...
                    case 74:
                        ...
                        FROM tblDepartures 
                        where tourID = {$tourId}
                        and month(dStartDate) = (
                        SELECT month(dStartDate)
                        FROM tblDepartures 
                        where tourID = {$tourId} 
                        and month(dStartDate)>={$month} and year(dStartDate) >= {$this->year} limit 1
                        )
                        and year(dStartDate) = (
                        SELECT year(dStartDate)
                        FROM tblDepartures 
                        where tourID = {$tourId}
                        and month(dStartDate)>={$month} and year(dStartDate) >= {$this->year} limit 1
                        ) 
                    ...
                    case 75:
                        ...
                        $sql = "SELECT IF(dStartDate > {$tempDate}, dStartDate, {$tempDate}) as dStartDate
                        FROM tblDepartures 
                        where tourID = {$tourId} 
                        and {$tempDate} <= dEndDate and dEndDate >= CURDATE()   
                        limit 1";
    ...
    {        
        ...
        switch ($this->tourType) {
            ...
            case 74:
                ...
                $year = date("Y");
                $sql = "SELECT distinct month(dStartDate) as m
                FROM tblDepartures 
                where tourID = $tourId 
                AND year(dStartDate) = $year";
                ...
                while ($row = $sql->fetch_assoc()) {
                    ...
                    $startDate = "$year-".$row["m"]."-01";
                    array_push($departures, ['startDate' => $startDate, 'endDate' => $startDate]);                    
            ...
            case 75:
                ...
                $sql = "SELECT dStartDate,dEndDate FROM tblDepartures where tourID = $tourId and dEndDate >= '{$tempDate}'";
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    array_push($departures, ['startDate' => $row["dStartDate"], 'endDate' => $row['dEndDate']]);                     
