  ...
  jQuery( function() {
    ...
    if(jQuery( "#datepicker-2" ).length > 0){
      ...
      tourObject.isAnualCalendar = updateAnualCalendarFlag(tourObject.tourId);

      jQuery('#menu1').append('<div class="search-loader-outer"><div class="search-loader"></div></div>');
      if(tourObject.isAnualCalendar){
        ...
        jQuery( "#datepicker-2" ).datepicker({
          ...
          onChangeMonthYear: function (year,month,datepicker) {
            calendarDay   = 0;
            if(month <= tourObject.lastMonth  && year <= tourObject.lastYear){
  ...
  jQuery(document).on('click','.month_ul > li',function(e){
    ...
    jQuery('#datepicker-2').datepicker("setDate", new Date(calendarYear,localMonth,01));
    jQuery('#monthYearModal').modal('hide');
  });
  ...
  function createAnnualCalendar(res){
    if(res.status){
      ...
      jQuery.each(res.data.result, function( i, val ) {
        let m = new Date(i).getMonth();
        if(monthList.indexOf(m)== -1){
          monthList.push(m);
          monthRow[m] = [];
          annualCalMonth += '<th>'+monthArray[m].toUpperCase()+'</th>';
          monthRow[m].push({date:i,price:val.price,class:val.class});
        }else{
          monthRow[m].push({date:i,price:val.price,class:val.class});
        }
      ...
      jQuery.each(monthRow, function( ind, val ) {
        for (var i = 0; i <= 31; i++) {
          if(monthRow[ind][i] != null){
            tableRow[i].push({date:monthRow[ind][i].date,price:monthRow[ind][i].price,class:monthRow[ind][i].class});
          }
        }
      ...
      for (var j = 0; j < tableRow.length; j++) {
        if(tableRow[j].length > 0){
          let singleRow = tableRow[j];
          tr += '<tr>';
          for (var k = 0; k < monthList.length; k++) {

            
              tr += (singleRow[k] != null) ? '<td class="'+singleRow[k].class+'" >'+new Date(singleRow[k].date).getDate()+'<p>'+res.prefix +singleRow[k].price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==1) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==2) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==3) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==4) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==5) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==6) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==7) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==8) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==9) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==10) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
              // tr += (i==11) ? '<td class="'+val.class+'" >'+new Date(val.date).getDate()+'<p>'+res.prefix +val.price+'</p></td>':'<td class="border-none"></td>';
      ...
      //   annualCalMonth += '<th>'+monthArray[i].toUpperCase()+'</th>';
      // }
      ...
      <div class="table-box">
        <div class="container">
          ...
          <div class="calender-top">
            ...
            <div class="calender-title"> <span class="calender-year">2022</span> </div>
            ...
            <div class="row">
              <div class="col-md-4">
                 <div class="Tour-dates">
                    ...
                    <h5>5th August - <br> 15th August 2021</h5>
                 </div>
              </div>
              <div class="col-md-4">
      ...
      `;
      let parent  = jQuery( "#datepicker-2" ).closest('.col-md-8');
      jQuery( "#datepicker-2" ).remove();
  ...
  function getCalendarPrice(){
    ...
    jQuery.ajax({
      ...
      if(data.status){
        ...
        // if(data.endingDate != undefined && data.endingDate != null && data.endingDate != ''&& calendarMonth == new Date(data.endingDate).getMonth()+1 && calendarYear == (new Date(data.endingDate).getFullYear())){
        ...
        // if(data.endingDate != undefined && data.endingDate != null && data.endingDate != '' && calendarYear == (new Date(data.endingDate).getFullYear())){
        //   jQuery('.pull-right').addClass('disabled');
        // }
        ...
        // if(jQuery('.ui-datepicker-next').hasClass('ui-state-disabled')){
        //   jQuery('.ui-datepicker-next').css('display','none');
        // }
 
        // if(jQuery('.pull-right').hasClass('disable')){
        //   jQuery('.pull-right').css('display','none');
        // }

        // if(parseInt(jQuery('.year-month > span').text()) ==  new Date(data.endingDate).getFullYear()){
        //   jQuery('.pull-right').css('display','none');
        // }
  ...
  function openTourUrl(){
    ...
    let url = 'http://new.realrussia.co.uk/tours/tour-request?pax='+personCount+'&lang='+lng+'&ddate='+calendarDay+' '+ monthArray[calendarMonth-1]+' '+calendarYear+'&tname='+selectedTourName+'&tid='+selectedTourId+'&price='+icon[0].data+totalPrice+''; 
  ...
  jQuery('.tour-favourites div p').first().click(function(e){
    e.preventDefault();
    updateFavPageData();
    window.open(base_url+'/favorite-list/', '_self');
  });
  ...
  function genrateCustomHeaderHTML(res){
    ...
    if(res.date > 0 ){
      ...
      if(res.date_from !=undefined && res.date_from!= null && res.date_from !='0000-00-00' && res.date_to !=undefined && res.date_to!= null && res.date_to !='0000-00-00'){
        ...
        inputDate = monthArray[fromDateArray[1]]+' '+fromDateArray[0]+' - '+monthArray[toDateArray[1]]+' '+toDateArray[0];
      }
